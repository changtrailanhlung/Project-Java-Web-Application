...
public class LoadMainPageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArrayList<BlogPostDTO> listPost = dao.LoadAllPost();
            HttpSession session = request.getSession();
            session.setAttribute("listPost", listPost);
            //setting session to expiry in 300 mins
            session.setMaxInactiveInterval(300 * 60);
            for (BlogPostDTO blogPostDTO : listPost) {
                System.out.println("LoadMainPageServlet " + blogPostDTO.getTittle());
            }
        } catch (Exception e) {
        } finally {
